...
from lsapy.functions.membership import (
    _check_fitting,  # noqa: PLC2701
    _get_best_fit,  # noqa: PLC2701
    _get_function_p0,  # noqa: PLC2701
    _rmse,  # noqa: PLC2701
    fit_membership,
)
...
class TestRMSE:
    ...
    def test_non_zero(self):
        ...
        assert _rmse(y_true, y_pred) == 10.0

    def test_nan(self):
        y_true = np.array([1, 2, 3, 4, 5])
        y_pred = np.array([np.nan, 2, 3, 4, 5])
        assert np.isnan(_rmse(y_true, y_pred))
...
class TestCheckFitting:
    ...
    def test_list(self):
        ...
        valid, skipped = _check_fitting(["logistic", 1])
    ...
    def test_invalid(self):
        ...
        with pytest.raises(ValueError, match="No functions to fit. Try to modify `fit_on` parameter."):
            _check_fitting("invalid")


class TestGetFunctionP0:
    def test_sigmoid(self):
        p0 = _get_function_p0("vetharaniam2022_eq5", np.array([1, 2, 3, 4, 5]))
        assert p0 == [1, 3]

    def test_gaussian(self):
        p0 = _get_function_p0("vetharaniam2024_eq10", np.array([1, 2, 3, 4, 5]))
        assert p0 == [1, 3, 1]

    def test_invalid(self):
        p0 = _get_function_p0("invalid", np.array([1, 2, 3, 4, 5]))
        assert p0 == []
...
class TestGetBestFit:
    def test_simple(self):
        ...
        rmse = np.array([3, 1, 2])
        params = [(1, 1), (2, 1), (3, 1)]
        func, param = _get_best_fit(functions, rmse, params)
    ...
    def test_nan(self):
        ...
        f, p = _get_best_fit(functions, rmse, params)
...
class TestFitMembership:
    ...
    def test_skipped(self):
        with pytest.warns(UserWarning, match="No methods to fit. Skipping: vetharaniam2024_eq8, vetharaniam2024_eq10."):
            fit_membership([-10, -6.5, -5, -3.5, 0], fit_on="gaussian_like")
